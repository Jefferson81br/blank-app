...
def renderizar_tela(supabase, user):
    ...
    with centro:
        ...
        data_sel = st.date_input("Data do Movimento", value=date.today(), max_value=date.today(), key="dt_mov")
        st.write("---")
        ...
        st.markdown(f"<div style='background-color:#1a1a1a; padding:10px; border-radius:5px; border:1px solid #333;'><b>SUBTOTAL ENTRADAS:</b> Sistema R$ {t_s_ent:,.2f} | <span style='color:#00ff00;'>Conf. R$ {t_c_ent:,.2f}</span> | <span style='color:#ff4b4b;'>Acerto R$ {t_a_ent:,.2f}</span></div>", unsafe_allow_html=True)
    ...
    with coluna_avisos:
        ...
        if fb.data:
            ...
            st.write("Nenhum feedback.")

        with st.form("f_final_envio", clear_on_submit=True):
            ...
            if st.form_submit_button("✅ SALVAR FECHAMENTO", use_container_width=True):
                ...
                res = db.salvar_fechamento(supabase, dados)
                if res[0]:
                    if imgs:
