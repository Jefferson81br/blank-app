...
def renderizar_tela(supabase, user):
    ...
    # --- NOVIDADE: SINALIZAÇÃO DE DATAS PREENCHIDAS ---
    st.subheader("📅 Status dos Últimos 7 Dias")
    
    # Buscamos os fechamentos da loja nos últimos 7 dias
    ...
    if res_check and res_check.data:
        datas_preenchidas = [d['data_fechamento'] for d in res_check.data]

    # Criamos colunas para mostrar os dias e os ícones
    cols_status = st.columns(7)
    for i in range(7):
        dia_analise = date.today() - timedelta(days=i)
        dia_str = str(dia_analise)
        with cols_status[6-i]: # Inverte para mostrar do mais antigo para o mais novo
            cor = "🟢" if dia_str in datas_preenchidas else "🔴"
            st.markdown(f"<div style='text-align:center; font-size:12px;'>{dia_analise.strftime('%d/%m')}<br>{cor}</div>", unsafe_allow_html=True)
    
    st.write("---")

    # Seleção de Data
    data_sel = st.date_input("Selecione a Data do Movimento", value=date.today(), max_value=date.today())
    
    # Validação imediata se a data já existe
    if str(data_sel) in datas_preenchidas:
        st.warning(f"⚠️ Já existe um fechamento salvo para o dia {data_sel.strftime('%d/%m/%Y')}. Se salvar novamente, os dados antigos serão atualizados.")
    ...
    # (A função linha_f e os grupos de Entradas/Saídas continuam iguais abaixo...)
    def linha_f(label, key, bloqueia_sistema=False):
        ...
        val_sis = col_sis.number_input("R$", key=f"s_{key}", format="%.2f", step=0.01, label_visibility="collapsed", disabled=bloqueia_sistema)
        val_conf = col_conf.number_input("R$", key=f"c_{key}", format="%.2f", step=0.01, label_visibility="collapsed")
    ...
    # ... (Cálculos totais e salvamento permanecem os mesmos) ...
    ...
    with st.form("f_final", clear_on_submit=True):
        arquivos = st.file_uploader("Anexar Prints", accept_multiple_files=True, type=['png', 'jpg', 'jpeg'])
