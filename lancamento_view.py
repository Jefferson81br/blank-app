...
def renderizar_tela(supabase, user):
    # --- AJUSTE DE LARGURA: 0.1 (margem), 2 (formulário), 2 (instruções/feedback) ---
    ...
    # --- COLUNA DA DIREITA (INSTRUÇÕES E FEEDBACK) ---
    with coluna_avisos:
        st.markdown("<br><br><br>", unsafe_allow_html=True)
        
        # 1. QUADRO DE INSTRUÇÕES
        st.info("""
        ### 📖 Instruções
        1. Confira os valores do sistema com atenção.
        2. Anexe os prints de fechamento de lote.
        3. Justifique qualquer acerto vermelho no campo de observações.
        """)

        st.write("---")

        # 2. QUADRO DE FEEDBACK (RÉPLICAS)
        st.subheader("💬 Feedback do Financeiro")
        fb_res = supabase.table("fechamentos")\
        ...
        if fb_res.data:
            for fb in fb_res.data:
                with st.container(border=True):
                    dt = date.fromisoformat(fb['data_fechamento']).strftime('%d/%m')
                    st.caption(f"Ref. dia {dt}")
                    st.markdown(f"**Gestor:** {fb['replica_gestor']}")
                    if fb['observacoes']:
                        st.markdown(f"*Sua Obs:* {fb['observacoes']}")
        else:
            st.write("Nenhum apontamento recente.")

        # --- FORMULÁRIO DE ENVIO (Sempre visível na direita) ---
        st.write("---")
        with st.form("f_final_new", clear_on_submit=True):
            imgs = st.file_uploader("Prints do Dia", accept_multiple_files=True, type=['png', 'jpg', 'jpeg'])
            obs_texto = st.text_area("Suas Observações / Justificativas", height=150)
            ...
            if st.form_submit_button("✅ SALVAR FECHAMENTO", use_container_width=True):
                dados = {
                    ...
                    "conf_outros": c_out, "observacoes": obs_texto
                }
